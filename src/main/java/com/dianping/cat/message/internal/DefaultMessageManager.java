...
public class DefaultMessageManager  implements MessageManager{
	...
	public void flush(MessageTree tree, boolean clearContext) {
		MessageSender sender = defaultTransportManager.getSender();
		...
		if (sender != null && isMessageEnabled()) {
			...
			if (m_throttleTimes % 10000 == 0 || m_throttleTimes == 1) {
				log.info("Cat Message is throttled! Times:" + m_throttleTimes);
			}
		}
	}
	...
	public void initialize()  {
		m_domain = defaultClientConfigManager.getDomain();
		...
		try {
			messageIdFactory.initialize(m_domain.getId());
		} catch (Exception e) {
			log.error("error when create mark file", e);
		}

		// initialize the tagged transaction cache
		final int size = defaultClientConfigManager.getTaggedTransactionCacheSize();
	...
	public boolean isCatEnabled() {
		return m_domain != null && m_domain.isEnabled() && defaultClientConfigManager.isCatEnabled();
	}

	@Override
	public boolean isMessageEnabled() {
		return m_domain != null && m_domain.isEnabled() && m_context.get() != null && defaultClientConfigManager.isCatEnabled();
	}
	...
	public void reset() {
		...
		if (ctx != null) {
			...
			if (tree != null) {
				double samplingRate = defaultClientConfigManager.getSampleRatio();
