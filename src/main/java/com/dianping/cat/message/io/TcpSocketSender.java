...
@Component
@Slf4j
public class TcpSocketSender implements Task, MessageSender {
	...
	public void initialize(List<InetSocketAddress> addresses) {
		...
		m_channelManager = new ChannelManager(addresses, clientConfigManager, messageIdFactory);
		...
		Runtime.getRuntime().addShutdownHook(new Thread() {
			@Override
			public void run() {
				log.info("shut down cat client in runtime shut down hook!");
				shutdown();
			}
		});
	...
	private void logQueueFullInfo(MessageTree tree) {
		...
		if (count % 1000 == 0 || count == 1) {
			log.error("Message queue is full in tcp socket sender! Count: " + count);
		}
	...
	private void offer(MessageTree tree) {
		if (clientConfigManager.isAtomicMessage(tree)) {
		...
		while (true) {
			...
			if (channel != null) {
				try {
					...
					log.error("Error when sending message over TCP socket!", t);
				}
			} else {
				try {
					Thread.sleep(5);
	...
	public void send(MessageTree tree) {
		if (!clientConfigManager.isBlock()) {
			double sampleRatio = clientConfigManager.getSampleRatio();
	...
	public void sendInternal(ChannelFuture channel, MessageTree tree) {
		if (tree.getMessageId() == null) {
			tree.setMessageId(messageIdFactory.getNextId());
		}
